    ...
    for i, line in enumerate(lines, 1):
        ...
        try:
            ...
            for subject_el in subjects:
                if subject_el and "OpenAI - Access Deactivated" in subject_el.get_text():
                    matched_subject = subject_el
                    break
            
            found_ban = matched_subject is not None
            
            if not found_ban:
                print(f"账户: {acc_pass}----正常! ")
                print(f"{Colors.GREEN}  正常: {acc_pass}")
            else:
                result_line = f"{acc_pass}----封禁"
                results.append(result_line)
                print(f"{Colors.RED}  封禁: {acc_pass}{Colors.END}")
